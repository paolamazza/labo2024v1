...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp/Under_0.9"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow/Under_0.9"
...
TS_strategy_guantesblancos_202109 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$train$undersampling <- 0.90
...
ZZ_final_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_desde <-  12000L
...
corrida_guantesblancos_202109 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  ZZ_final_guantesblancos( "ZZ0001_extraenv", c("HT0001","TS0001") )
...
corrida_guantesblancos_202109( "gb01_extraenv" )
